...
class CRMs_List extends WP_List_Table {
	...
	public function no_items() {
		_e( 'No leilabidad_crm avaliable.', 'sp' );
	}
	/**
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'date':
				return leilabidad_jdate_dashed($item[ $column_name ]);
            case 'time':
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'leilabidad_crm_per_page', 25 );
		...
		$this->items = self::get_leilabidad_crm( $per_page, $current_page );
	}
				...
				<div id="post-body" class="metabox-holder columns-2">
					<div id="post-body-content">
						<div class="meta-box-sortables ui-sortable">
								...
								$this->leilabidad_crm_obj->prepare_items();
								
echo '<div id="tblExport">';
								
								$this->leilabidad_crm_obj->display();
	echo '</div>';
 }								?>
							</form>
	...
	public function screen_option() {
		$option = 'per_page';
		$args   = [
			'label'   => 'leilabidad_crm',
			'default' => 25,
			'option'  => 'leilabidad_crm_per_page'
		];
		add_screen_option( $option, $args );
		$this->leilabidad_crm_obj = new CRMs_List();
	}
	/** Singleton instance */
...
$html.='</div>
 ...
 add_shortcode('leilabidad_CRM', 'CRM_function');
...
function html_search(){
	...
	global $wpdb; 
		...
		$sql="SELECT * FROM {$wpdb->prefix}leilabidad_crm WHERE mobile='$mobile_sch'	AND author='$the_user_id'";
...
$html_search.='</div>';
					...
					foreach ( $res_1 as $res1 ) 
					{
						...
						$html_search.= '<div class="col-md-1">'.leilabidad_jdate_dashed($res1->date).'</div></div>';	
...
function html_report(){
	...
	$table_crm = $wpdb->prefix."leilabidad_crm";
		...
		$ar_id=array();
		$sql="SELECT * 	FROM {$wpdb->prefix}leilabidad_crm	WHERE  author='$the_user_id'";
	if(($_POST['from']) &&  ($_POST['to']) ) {
...
$html_report.='</div>';
					...
					foreach ( $res_1 as $res1 ) 
					{
						...
						$html_report.= '<div class="col-md-1">'.leilabidad_jdate_dashed($res1->date).'</div></div>';	
